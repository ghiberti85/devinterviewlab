...
function ScoreBar({ value, label }: { value: number; label: string }) {
  ...
  return (
    <div className="flex items-center justify-between gap-3">
      ...
      <span className="text-xs tabular-nums w-7 text-right font-medium">{v}</span>
    </div>
  )
}
...
function ScoreCircle({ score }: { score: number }) {
  ...
  const color = v >= 75 ? 'text-green-600 dark:text-green-400'
...
export function AIFeedbackPanel({ evaluation, title }: Props) {
  ...
  const { strengths, gaps, suggestions, score_breakdown, star_analysis } = feedback
      ...
      <div className="p-5 border rounded-xl bg-muted/30 space-y-4">
        <div className="flex items-center gap-6">
          ...
          <div className="text-sm text-muted-foreground">
            ...
            <p className="text-xs mt-0.5">
              ...
              {' '}
              {Number(score) >= 75 ? (t.common.language === 'pt' ? 'Boa resposta' : 'Strong answer')
                : Number(score) >= 50 ? (t.common.language === 'pt' ? 'Resposta parcial' : 'Partial answer')
                : (t.common.language === 'pt' ? 'Precisa melhorar' : 'Needs improvement')}
            </p>
          </div>
        </div>
        ...
        {score_breakdown && Object.keys(score_breakdown).length > 0 && (
          <div className="space-y-2.5">
            {(Object.entries(score_breakdown) as [string, number][]).map(([k, v]) => (
              <ScoreBar
                key={k}
                value={Number(v)}
      ...
      {star_analysis && (
        <div>
          ...
          <div className="grid grid-cols-2 gap-2">
            {(Object.entries(star_analysis) as [string, any][]).map(([key, val]) => (
              <div key={key} className="border rounded-lg p-3 space-y-1">
                <div className="flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-1.5">
                    ...
                    <span className="text-xs tabular-nums font-medium">{Number(val.score)}</span>
                  </div>
                </div>
