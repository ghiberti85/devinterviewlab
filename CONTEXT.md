...
- **Garantia contra cache em qualquer camada** *(2026-07-05)*, motivada por relato de PWA continuar mostrando dado antigo mesmo após as correções acima:
  - `GET /api/topics` tem `export const dynamic = 'force-dynamic'` + `Cache-Control: no-store, must-revalidate` explícito na resposta (redundante com o `next.config.js`, que já força `no-store` em todo `/api/*` — mantido como defesa em profundidade)
  - `useTopics()`: `fetch(..., { cache: 'no-store' })`, `staleTime: 60s` (era 10min), `refetchOnMount: 'always'`, `refetchInterval: 30s`
  - Motivo do `refetchInterval`: um PWA em modo standalone pode retomar de um estado suspenso sem disparar os eventos `focus`/`visibilitychange` de que o React Query depende para `refetchOnWindowFocus` — sem isso, a aba pode nunca re-buscar dados novos enquanto o app não for reaberto do zero. O polling de 30s garante convergência independente de esses eventos disparando ou não, e pausa automaticamente quando a aba/app está de fato em segundo plano
