...
export function LoginClient({ error, message, redirectPath }: LoginClientProps) {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4">
      <div className="w-full max-w-sm space-y-6">
        ...
        <div className="border rounded-lg p-6 space-y-4 bg-card">
          <h2 className="font-semibold text-lg">{t.signIn}</h2>
          ...
          <p className="text-center text-sm text-muted-foreground">
            {t.noAccount}{' '}
            <Link href="/register" className="text-primary hover:underline">{t.createSubmit}</Link>
          </p>
        </div>

        {/* Demo link */}
        <div className="text-center">
          <p className="text-xs text-muted-foreground mb-2">{t.demoPrompt}</p>
          <Link
            href="/demo"
