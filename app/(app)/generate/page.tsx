...
import {
  useSavedCV, useDocuments, useUploadDocument,
  useDeleteDocument, useUpdateDocument, formatFileSize,
  type UserDocument
...
import {
  Upload, Sparkles, Check, FileText, X, Save,
  ChevronDown, ChevronUp, RefreshCw, Trash2,
  BookUser, FilePlus, ToggleLeft, ToggleRight
} from 'lucide-react'
...
interface GeneratedQuestion {
  title: string; body: string | null; ideal_answer: string
  difficulty: Difficulty; is_behavioral: boolean; detected_skills?: string[]
}
interface GenerateResult {
  questions: GeneratedQuestion[]; skills_detected: string[]; summary: string
}
...
export default function GeneratePage() {
  ...
  // ── Saved CV ────────────────────────────────────────────────────────────────
  const { data: savedCV, isLoading: cvLoading, refetch: refetchCV } = useSavedCV()
  ...
  async function handleExtraFileAdd(e: React.ChangeEvent<HTMLInputElement>) {
    ...
    setTempFiles(prev => [...prev, ...files.map(f => ({ file: f, keepStored: false }))])
    e.target.value = ''
  }
  ...
  async function saveTempFile(i: number) {
    ...
    try {
      const result = await uploadDoc.mutateAsync({
        file: tf.file, docType: 'other', keepStored: true,
      })
  ...
  async function handleGenerate() {
    ...
    const hasExtra = tempFiles.length > 0 || selectedSavedIds.size > 0 || context.trim()
    ...
    setLoading(true); setError(''); setResult(null); setSelected(new Set()); setSavedCount(0)
    ...
    for (const i of selected) {
      ...
      try {
        await createQuestion.mutateAsync({
          title: q.title, body: q.body ?? undefined,
          ideal_answer: q.ideal_answer, difficulty: q.difficulty,
          is_behavioral: q.is_behavioral, language: questionLang,
        })
        saved++
      } catch {}
    }
    setSavedCount(saved); setSaving(false); setSelected(new Set())
  }
  ...
  const difficultyOptions = [
    ...
    { value: 'mixed',  label: language === 'pt' ? '🎲 Misto (varia)' : '🎲 Mixed (varies)' },
  ]

  const cvLabel   = language === 'pt' ? 'Currículo (CV)' : 'Resume (CV)'
  const autoLabel = language === 'pt' ? 'Incluído automaticamente em todas as gerações' : 'Automatically included in all generations'
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div className="border rounded-xl bg-card overflow-hidden">
        <div className="px-5 py-3 border-b bg-muted/30 flex items-center gap-2">
          ...
          <span className="text-xs text-muted-foreground ml-1">— {autoLabel}</span>
        </div>
              ...
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-xs text-muted-foreground">
                    {formatFileSize(savedCV.file_size)} · {savedCV.chars?.toLocaleString()} {language === 'pt' ? 'caracteres extraídos' : 'chars extracted'}
                    {' · '}
                    {language === 'pt' ? 'atualizado' : 'updated'}{' '}
                    {new Date(savedCV.updated_at ?? savedCV.created_at).toLocaleDateString('pt-BR')}
                  </p>
                </div>
              </div>
              ...
              {uploadDoc.isPending && uploadDoc.variables?.docType === 'cv' ? (
                ...
                <>
                  ...
                  <p className="text-sm font-medium">
                    {language === 'pt' ? 'Faça upload do seu CV (PDF)' : 'Upload your CV (PDF)'}
                  </p>
                  <p className="text-xs text-muted-foreground mt-1">
                    {language === 'pt' ? 'Será salvo e usado automaticamente em todas as gerações' : 'Saved and auto-used in all future generations'}
                  </p>
                </>
              )}
            </div>
        ...
        <div className="px-5 py-3 border-b bg-muted/30 flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <span className="text-xs text-muted-foreground">
              {language === 'pt' ? '(descrição da vaga, portfólio, etc.)' : '(job description, portfolio, etc.)'}
            </span>
          </div>
          <button
          ...
          <input ref={extraInputRef} type="file" accept="application/pdf" multiple className="hidden" onChange={handleExtraFileAdd} />
        </div>
        ...
        <div className="p-5 space-y-3">
          ...
          {savedOtherDocs.length > 0 && (
            <div className="space-y-2">
              ...
              {savedOtherDocs.map(doc => (
                <div key={doc.id} className={`flex items-center justify-between gap-3 p-3 border rounded-lg transition-colors ${selectedSavedIds.has(doc.id) ? 'border-primary/50 bg-primary/5' : 'hover:bg-accent'}`}>
          ...
          {tempFiles.length > 0 && (
            <div className="space-y-2">
              ...
              {tempFiles.map((tf, i) => (
                <div key={i} className="flex items-center justify-between gap-3 p-3 border rounded-lg bg-yellow-50/50 dark:bg-yellow-900/10 border-yellow-200 dark:border-yellow-800">
                  <div className="flex items-center gap-2 flex-1 min-w-0">
                    <FileText size={14} className="text-yellow-600 dark:text-yellow-400 shrink-0" />
                    <span className="text-sm truncate">{tf.file.name}</span>
                    <span className="text-xs text-muted-foreground">{formatFileSize(tf.file.size)}</span>
                  ...
                  <div className="flex items-center gap-2 shrink-0">
                    <button
                      ...
                      title={language === 'pt' ? 'Salvar permanentemente' : 'Save permanently'}
                    >
                      <Save size={11} />
                    </button>
                    <button onClick={() => removeTempFile(i)} className="text-muted-foreground hover:text-destructive">
                      <X size={13} />
                    </button>
                  </div>
                </div>
          ...
          <div>
            <label className="block text-xs font-medium text-muted-foreground mb-1.5">
              {language === 'pt' ? 'Descrição da vaga / contexto adicional' : 'Job description / additional context'}
            </label>
            <textarea
              value={context}
          ...
          {savedOtherDocs.length === 0 && tempFiles.length === 0 && !context && (
      ...
      <div className="border rounded-xl p-5 bg-card space-y-4">
        ...
        <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
          ...
          <div>
            ...
            <select value={categoryName} onChange={e => setCategoryName(e.target.value)}
              className="w-full border rounded-md px-3 py-2 text-sm bg-background focus:outline-none focus:ring-2 focus:ring-ring">
              <option value="">{language === 'pt' ? 'Detectar automaticamente' : 'Auto-detect'}</option>
        ...
        <div className="flex items-center justify-between pt-1">
          <label className="flex items-center gap-2 text-sm cursor-pointer">
            <input type="checkbox" checked={isBehavioral} onChange={e => setIsBehavioral(e.target.checked)} className="rounded" />
            {language === 'pt' ? 'Incluir questões comportamentais (STAR)' : 'Include behavioral questions (STAR)'}
          </label>
          <button
      ...
      {/* Sources summary */}
      {(savedCV || selectedSavedIds.size > 0 || tempFiles.length > 0 || context) && (
        <div className="flex flex-wrap gap-2 text-xs">
          <span className="text-muted-foreground">{language === 'pt' ? 'Fontes:' : 'Sources:'}</span>
          {savedCV && <span className="bg-green-100 dark:bg-green-900/30 text-green-800 dark:text-green-300 px-2 py-0.5 rounded-full">✅ CV: {savedCV.name}</span>}
          {selectedSavedIds.size > 0 && <span className="bg-blue-100 dark:bg-blue-900/30 text-blue-800 dark:text-blue-300 px-2 py-0.5 rounded-full">📁 {selectedSavedIds.size} doc{selectedSavedIds.size !== 1 ? 's' : ''} salvos</span>}
          {tempFiles.length > 0 && <span className="bg-yellow-100 dark:bg-yellow-900/30 text-yellow-800 dark:text-yellow-300 px-2 py-0.5 rounded-full">📄 {tempFiles.length} arquivo{tempFiles.length !== 1 ? 's' : ''} temp</span>}
          {context && <span className="bg-purple-100 dark:bg-purple-900/30 text-purple-800 dark:text-purple-300 px-2 py-0.5 rounded-full">📝 Contexto</span>}
        </div>
      )}

      {error && (
        <div className="text-sm bg-red-50 dark:bg-red-900/20 text-red-800 dark:text-red-300 border border-red-200 rounded-lg px-4 py-3">{error}</div>
      )}
      ...
      {loading && (
        <div className="border rounded-xl p-12 bg-card flex flex-col items-center gap-4 text-muted-foreground">
          ...
          <p className="text-sm">
            {language === 'pt' ? 'Analisando contexto e gerando questões personalizadas…' : 'Analysing context and generating personalised questions…'}
          </p>
        </div>
      )}
        ...
        <div className="space-y-4">
              ...
              <div className="flex items-center gap-3 shrink-0">
                <button onClick={toggleAll} className="text-xs text-muted-foreground hover:text-foreground">
                  {selected.size === result.questions.length
                    ? (language === 'pt' ? 'Desmarcar todos' : 'Deselect all')
                ...
                >
                  ...
                  {saving
                    ...
                    : (language === 'pt' ? `Salvar ${selected.size}` : `Save ${selected.size}`)}
                </button>
              </div>
            </div>
            ...
            {difficulty === 'mixed' && result.questions.length > 0 && (
              <div className="flex items-center gap-3 pt-1 border-t">
                ...
                {(['easy', 'medium', 'hard'] as Difficulty[]).map(d => {
                  const n = result.questions.filter(q => q.difficulty === d).length
                  return n > 0 ? (
          ...
          {savedCount > 0 && (
            <div className="flex items-center gap-2 text-sm bg-green-50 dark:bg-green-900/20 text-green-800 dark:text-green-300 border border-green-200 rounded-lg px-4 py-3">
              <Check size={15} />
              {language === 'pt' ? `${savedCount} questão(ões) salva(s) na biblioteca.` : `${savedCount} question(s) saved.`}
            </div>
          )}

          <div className="space-y-3">
            {result.questions.map((q, i) => {
              const isSel = selected.has(i); const isExp = expanded.has(i)
              return (
                <div key={i} className={`border rounded-xl bg-card transition-all ${isSel ? 'border-primary/50 shadow-sm' : 'opacity-60'}`}>
                  <div className="p-4 flex items-start gap-3">
                    <input type="checkbox" checked={isSel} onChange={() => toggleSelect(i)} className="mt-0.5 rounded shrink-0 cursor-pointer" />
                    <div className="flex-1 min-w-0">
                      <div className="flex items-center gap-2 flex-wrap mb-1.5">
                        <DifficultyBadge difficulty={q.difficulty} />
                        {q.is_behavioral && <span className="text-xs bg-purple-100 text-purple-800 dark:bg-purple-900/30 dark:text-purple-400 px-2 py-0.5 rounded-full">Comportamental</span>}
                      ...
                      <p className="text-sm font-medium leading-relaxed">{q.title}</p>
                      {q.body && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{q.body}</p>}
                    </div>
                    <button onClick={() => toggleExpand(i)} className="text-muted-foreground hover:text-foreground shrink-0 p-1">
                  ...
                  {isExp && (
                    <div className="border-t px-4 py-4 bg-muted/30 rounded-b-xl">
                      ...
                      <p className="text-sm whitespace-pre-wrap leading-relaxed">{q.ideal_answer}</p>
                    </div>
                  )}
                </div>
              )
